...
type EntryDoc = {
  ...
  // ✅ Official field (matches My Bracket)
  championshipScore?: ChampionshipScore;

  // ✅ Back-compat (older/other pages might have written this)
...
function TeamRowView({
  ...
  return (
    ...
    >
      ...
      <span style={{ fontSize: 14, fontWeight: selected ? 800 : 650, letterSpacing: 0.2, lineHeight: "16px" }}>
        {seed ? (
          <>
            <span style={{ opacity: 0.85 }}>{seed}</span>
            <span style={{ opacity: 0.6 }}> · </span>
...
export default function EntryBracketPage() {
  ...
  // ✅ FIX: pull prediction from championshipScore first, fallback to tiebreaker
  const tiebreakerInfo = useMemo(() => {
    const finalGame = games.find((g) => g.round === 5) || null;

    const predicted = entry?.championshipScore ?? entry?.tiebreaker ?? null;
    const aScore = predicted?.a ?? null;
    const bScore = predicted?.b ?? null;

    if (!finalGame) {
      return { aName: "TBD", bName: "TBD", aScore, bScore };
    }
    ...
    return { aName, bName, aScore, bScore };
  }, [games, picks, inc, teamMap, entry]);
  ...
  return (
    <main style={{ padding: 24, fontFamily: "system-ui", color: "#fff" }}>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 12, flexWrap: "wrap" }}>
          <h1 style={{ margin: 0 }}>Bracket — {entry?.displayName || entry?.id || "Unknown"}</h1>
          <Link href="/leaderboard" style={{ opacity: 0.85, color: "#fff" }}>
            ← Back to Leaderboard
          </Link>
        </div>
      ...
      {/* ✅ Tie-breaker display (now synced) */}
      <div
        style={{
          marginTop: 10,
          border: "1px solid rgba(255,255,255,0.12)",
      ...
      >
        ...
        <div style={{ opacity: 0.85, lineHeight: "20px" }}>
          ...
          {typeof tiebreakerInfo.aScore !== "number" || typeof tiebreakerInfo.bScore !== "number" ? (
            <span style={{ marginLeft: 10, opacity: 0.65 }}>(not provided)</span>
          ) : null}
        </div>
      </div>
      ...
      >
        <div style={{ position: "relative", width: CANVAS_W, height: CANVAS_H }}>
          ...
          {positionedGames.map(({ game, x, y }) => {
            ...
            return (
              <div key={game.id} style={{ position: "absolute", left: x, top: y }}>
                <GameCardView game={game} teamMap={teamMap} picks={picks} activeRound={activeRound} aId={aId} bId={bId} />
              </div>
            );
          })}
